...
// Time until midnight (for display)
      ...
      <PageTransition>
        <div className="space-y-4 max-w-2xl mx-auto">
          ...
          >
            ...
            <p className="text-[#8590a6] text-sm mb-6">
              {subject.credits} credits · Goal: {subject.attendance_goal}%
            </p>
            ...
            <p className="text-xs text-[#8590a6] mt-2">
              {subject.attended_classes} of {subject.total_classes} classes attended
            </p>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Card padding="md">
              ...
              <p className={`text-3xl font-bold ${mustAttend > 0 ? 'text-[#ba1a1a]' : 'text-[#24a375]'}`}>
                {mustAttend}
              </p>
          ...
          {/* TODAY'S ATTENDANCE — day-wise, one submission only */}
          <Card>
            ...
            <AnimatePresence mode="wait">
              {alreadyMarkedToday ? (
                // Already marked — show result, locked until midnight
                <motion.div
                  key="marked"
                  ...
                  className={`rounded-xl p-4 flex items-center justify-between ${
                    todayRecord.status === 'present'
                      ? 'bg-[#f0fdf4] border border-[#85f8c4]'
                      : 'bg-[#fff5f5] border border-[#ffdad6]'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className={`font-semibold text-sm ${
                        todayRecord.status === 'present' ? 'text-[#24a375]' : 'text-[#ba1a1a]'
                      }`}>
                        {todayRecord.status === 'present' ? 'Present today' : 'Absent today'}
                      </p>
                      <p className="text-xs text-[#75777d]">Marked for today · resets after midnight</p>
                    </div>
                  </div>
                ...
                >
                  <div className="grid grid-cols-2 gap-3">
                    <motion.button
                      ...
                      onClick={() => markMutation.mutate({ status: 'present' })}
                      disabled={markMutation.isPending}
                      className="flex items-center justify-center gap-2 bg-[#85f8c4] text-[#002114] py-3.5 rounded-xl font-semibold text-sm hover:bg-[#68dba9] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {markMutation.isPending && markMutation.variables?.status === 'present' ? (
                    ...
                    <motion.button
                      whileTap={{ scale: 0.96 }}
                      whileHover={{ scale: 1.02 }}
                      onClick={() => markMutation.mutate({ status: 'absent' })}
                      disabled={markMutation.isPending}
                      className="flex items-center justify-center gap-2 bg-[#ffdad6] text-[#93000a] py-3.5 rounded-xl font-semibold text-sm hover:bg-[#ffb4ab] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {markMutation.isPending && markMutation.variables?.status === 'absent' ? (
                        <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                        </svg>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${
                          record.status === 'present' ? 'bg-[#24a375]' :
                          record.status === 'proxy' ? 'bg-[#505f76]' : 'bg-[#ba1a1a]'
                        }`} />
